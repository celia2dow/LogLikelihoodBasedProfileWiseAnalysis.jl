...
function sum_loglikelihood(times,params_combined,ICs,ode_system,data,error_type)
    ...
    y=odesolver(times,pars,ICs,ode_system);  # Find the model solution for the given parameter values
    e=0;                                    # Initialise sum of loglikelihood values

    if error_type == "Normal"
        Sd = params_combined[end]
        model_params = params_combined[1:end-1]
        data_dists=[Normal(mi,Sd) for mi in y];     # Generate Normal distributions for each times point
